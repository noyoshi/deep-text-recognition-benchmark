...
class Matcher(object):
    def __init__(self,
        ...
        self.true_word_list = list(true_word_list)
        ...
        if possible_true_words_dict is None:
            self.possible_true_words_dict = { 
                    word:self.find_matches(word) for word in self.true_word_list 
            }
        else:
            self.possible_true_words_dict = possible_true_words_dict

        self.true_chars = collections.Counter()
        self.parsed_chars = collections.Counter()

        self.char_level_accuracy = self._get_char_level_accuracy()

    def _get_char_level_accuracy(self):
        """Computes just how many chars we have that are the same, only cares
        about lowercase ascii characters, and does not care about position"""
        import string
        for parsed_word in self.parsed_word_list:
            if parsed_word is None: continue
            parsed_word = parsed_word.lower()

            for c in parsed_word:
                if c in string.ascii_lowercase:
                    self.parsed_chars[c] += 1

        for word in self.true_word_list:
        ...
        for c in string.ascii_lowercase:
            guessed = self.parsed_chars[c]
            actual  = self.true_chars[c]
            total += actual
            correct_n = min(guessed, actual) # how many did we get right?
            incorrect_n = abs(guessed - actual)
            correct += correct_n
